...
export function MagneticText({ 
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn("relative inline-flex items-center justify-center cursor-none select-none px-12 py-6 rounded-full border border-primary/20 bg-primary/5 hover:bg-primary/10 transition-colors overflow-hidden", className)}
    >
      {/* Base text layer - original text */}
      <span className={cn("text-xl md:text-2xl font-bold tracking-tight text-foreground/80 whitespace-nowrap", textClassName)}>{text}</span>
      ...
      >
        ...
        >
          <span className={cn("text-xl md:text-2xl font-bold tracking-tight text-primary-foreground whitespace-nowrap", textClassName)}>
            {hoverText || text}
          </span>
        </div>
      </div>
